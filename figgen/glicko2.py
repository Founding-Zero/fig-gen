...
class GlickoCalc:
    ...
    # def rating_update(self, p, num):
    #     model_name = ["nanogpt", "stockfish"]
    #     self.session_state[f"{model_name[num]}_rating"] = round(p.mu)
    #     self.session_state[f"{model_name[num]}_rd"] = round(p.phi)
    #     self.session_state[f"{model_name[num]}_v"] = round(p.sigma, 8)
    ...
    def glicko2_update(self, result, stockfish_level):
        ...
        r2 = env.create_rating(
            self.stockfish_elos[str(stockfish_level)],
            self.session_state["stockfish_rd"],
            self.session_state["stockfish_v"],
        )

        p = [None, None]
        if result_options[int(result)] == "#1 wins":  # nanogpt won
            p[0], p[1] = env.rate_1vs1(r1, r2, drawn=False)
        elif result_options[int(result)] == "#2 wins":  # stockfish won
